...
class AlipayView(APIView):
    def get(self,request):
        ...
        for key, value in request.POST.items():
            processd_dict[key] = value

        sign = processd_dict.pop("sign", None)
        alipay = AliPay(
        ...
        verify_re = alipay.verify(processd_dict, sign)
        if verify_re is True:
            order_sn = processd_dict.get('out_trade_no', None)
            trade_no = processd_dict.get('trade_no', None)
            trade_status = processd_dict.get('trade_status', None)
